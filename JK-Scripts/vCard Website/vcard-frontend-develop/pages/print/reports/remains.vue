              ...
              <table class="table b-table table-striped table-bordered">
                ...
                <tbody>
                  ...
                  <tr v-for="result in results" :key="result.id">
                    ...
                    <td>{{ result.number }}</td>
                    <td>{{ result.title }}</td>
                    <td>{{ $getFormattedPrice(result.revenueRemain) }}</td>
                    <td>{{ $getFormattedPrice(result.balanceRemain) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
export default {
  ...
  async asyncData({ $axios, $dayjs, $getErrorMessage, route, store }) {
    try {
      ...
      const { data } = await $axios.post('/reports/locations/remains', {
        filters: {
          ...filters,
