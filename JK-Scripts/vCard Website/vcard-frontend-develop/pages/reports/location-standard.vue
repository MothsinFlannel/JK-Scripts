<template>
  <div>
    <div class="mb-4">
      <h3 class="mb-0">Location Standard Report</h3>
      ...
      <Breadcrumb
        :items="[
          ...
          {
            title: 'Location Standard Report',
          },
        ]"
      />
    </div>
    ...
    <b-card class="mb-4 shadow-sm" header-class="pt-1">
      <template #header>
        ...
        >
          <span class="mt-2">
            {{ `Location Standard Report: ${since} — ${until}` }}
          </span>
          <div
  ...
  methods: {
    async exportDevices() {
      try {
        ...
        if (typeof data === 'string') {
          ...
          saveAs(blob, 'location-standard.csv')
        }
      } catch (error) {
        console.error(error)
      } finally {
    ...
    printReport() {
      ...
      this.printIframe.setAttribute(
        'src',
        `${window.origin}${this.$router.options.base}print/reports/location-standard${window.location.search}`
      )
      this.printIframe.setAttribute(
        'style',
