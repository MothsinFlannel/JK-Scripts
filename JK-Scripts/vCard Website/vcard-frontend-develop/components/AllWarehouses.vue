<template>
  <b-card class="mb-4 shadow-sm">
    <template #header>
      ...
      >
        <p class="m-0">vCard Warehouses</p>
        <div
    ...
    >
      <div class="row mr-auto">
        <div class="col-12 d-flex flex-column">
          ...
          <b-pagination
            v-if="
              $route.query.limit > 0 && warehousesCount > +$route.query.limit
            "
            :value="+$route.query.page"
            :total-rows="warehousesCount"
      ...
      <div
        v-if="$isAllowed('app/warehouses/create')"
    ...
    <scroll-wrapper ref="scroll-wrapper">
      ...
      >
        <template #cell(name)="data">
          <nuxt-link :to="`/warehouses/${data.item.id}`">
            {{ data.item.name }}
          </nuxt-link>
        </template>
    ...
    <div v-if="$route.query.limit > 0" class="app-card__table-footer">
      <div class="mt-3 mr-3 mb-1 mb-sm-0">
        ...
        to
        {{
          Math.min(+$route.query.page * +$route.query.limit, warehousesCount)
        }}
        of {{ warehousesCount }} entries
      </div>

      <b-pagination
        v-if="warehousesCount > +$route.query.limit"
        :value="+$route.query.page"
        :total-rows="warehousesCount"
        :per-page="+$route.query.limit"
        first-text="First"
    ...
    <WarehouseModal
      ref="warehouse-modal"
      @updated="getWarehouses"
    ></WarehouseModal>
  </b-card>
</template>

<script>
import WarehouseModal from '@/components/modals/Warehouse'
...
export default {
  ...
  methods: {
    async getWarehouses() {
      try {
        ...
        if (data.success) {
          ...
          this.warehousesCount = data.count
        }
    ...
    deleteWarehouse(warehouse) {
      this.$bvModal
        .msgBoxConfirm(`Are you sure you want to delete ${warehouse.name}?`, {
    ...
    addWarehouse() {
      this.$refs['warehouse-modal'].showModal(null)
    },
    editWarehouse(terminal) {
      this.$refs['warehouse-modal'].showModal(terminal)
    },
  },
}
</script>
