<template>
  ...
  >
    ...
    <div class="row">
      <div class="col-12">
        <b-form-group label="Machine type name">
          <b-form-input
...
export default {
  ...
  methods: {
    showModal(warehouse) {
      ...
      this.$v.$reset()
      this.$refs['machine-types-modal'].show()
    },
    async onSubmit() {
      this.error = null
      this.$v.$touch()
        ...
        try {
          ...
          const { data } = await this.$axios.post(
            typeof this.warehouse.id === 'number'
              ? '/app/machine-types/update'
              : '/app/machine-types/create',
            requestData
          )
          ...
          if (data.success) {
            ...
            this.$refs['machine-types-modal'].hide()
          }
