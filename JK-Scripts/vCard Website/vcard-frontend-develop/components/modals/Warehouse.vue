<template>
  <b-modal
    ref="warehouse-modal"
    :title="
      typeof warehouse.id === 'number' ? 'Edit warehouse' : 'Add warehouse'
    "
  ...
  >
    ...
    <div class="row">
      <div class="col-12">
        <b-form-group label="Warehouse name">
          <b-form-input
...
export default {
  ...
  methods: {
    showModal(warehouse) {
      ...
      this.$v.$reset()
      this.$refs['warehouse-modal'].show()
    },
    async onSubmit() {
      this.error = null
      this.$v.$touch()
        ...
        try {
          ...
          const { data } = await this.$axios.post(
            typeof this.warehouse.id === 'number'
              ? '/app/warehouses/update'
              : '/app/warehouses/create',
            requestData
          )
          ...
          if (data.success) {
            ...
            this.$refs['warehouse-modal'].hide()
          }
