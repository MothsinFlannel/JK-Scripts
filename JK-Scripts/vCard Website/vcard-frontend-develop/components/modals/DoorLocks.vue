<template>
  <b-modal
    ...
    :title="
      typeof warehouse.id === 'number' ? 'Edit door lock' : 'Add door lock'
    "
...
export default {
  ...
  methods: {
    showModal(warehouse) {
      ...
      this.$v.$reset()
      this.$refs['door-locks-modal'].show()
    },
    async onSubmit() {
      this.error = null
      this.$v.$touch()
        ...
        try {
          ...
          const { data } = await this.$axios.post(
            typeof this.warehouse.id === 'number'
              ? '/app/door-locks/update'
              : '/app/door-locks/create',
            requestData
          )
          ...
          if (data.success) {
            ...
            this.$refs['door-locks-modal'].hide()
          }
