  ...
  <b-card class="mb-4 shadow-sm">
    ...
    >
      <div class="row mr-auto">
        <div class="col-12 d-flex flex-column">
          ...
          <b-pagination
            v-if="$route.query.limit > 0 && routesCount > +$route.query.limit"
            :value="+$route.query.page"
            :total-rows="routesCount"
      ...
      <div class="d-flex align-items-center align-self-end mt-3">
        <b-button
          v-if="$isAllowed('app/routes/create')"
          type="button"
    ...
    <scroll-wrapper ref="scroll-wrapper">
      ...
      >
        <template #cell(actions)="data">
          ...
          >
            <b-button
              v-if="$isAllowed('app/routes/update')"
              :to="`/routes/${data.item.id}/settings`"
              type="button"
              variant="outline-light"
              size="sm"
            ...
            <b-button
              v-if="$isAllowed('app/routes/delete')"
              target="_blank"
              type="button"
              variant="outline-light"
              size="sm"
    ...
    <div v-if="$route.query.limit > 0" class="app-card__table-footer">
      <div class="mt-3 mr-3 mb-1 mb-sm-0">
        ...
        to
        {{ Math.min(+$route.query.page * +$route.query.limit, routesCount) }}
        of {{ routesCount }} entries
      </div>

      <b-pagination
        v-if="routesCount > +$route.query.limit"
        :value="+$route.query.page"
        :total-rows="routesCount"
        :per-page="+$route.query.limit"
        first-text="First"
...
export default {
  ...
  computed: {
    ...
    routesTableFields() {
      ...
      if (
        this.$isAllowed('app/routes/update') ||
        this.$isAllowed('app/routes/delete')
      ) {
        routesTableFields.push({
          key: 'actions',
          label: 'Manage',
          tdClass: 'py-0',
          thStyle: { width: '1%' },
  ...
  methods: {
    ...
    deleteRoute(route) {
      this.$bvModal
        .msgBoxConfirm(`Are you sure you want to delete ${route.name}?`, {
          title: 'Delete Route',
        ...
        .then(async (value) => {
          if (value) {
            try {
              ...
              const { data } = await this.$axios.post('/app/routes/delete', {
                id: route.id,
              })
