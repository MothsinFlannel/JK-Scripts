<template>
  <b-card class="mb-4 shadow-sm">
    <template #header>
      ...
      >
        <p class="m-0">vCard Machine Types</p>
        <div
    ...
    >
      <div class="row mr-auto">
        <div class="col-12 d-flex flex-column">
          ...
          <b-pagination
            v-if="
              $route.query.limit > 0 && machineTypesCount > +$route.query.limit
            "
            :value="+$route.query.page"
            :total-rows="machineTypesCount"
        ...
        <template #cell(actions)="data">
          ...
          >
            <EditButton
              ...
              @click="editMachineType(data.item)"
            />

            <DeleteButton @click="deleteMachineType(data.item)" />
          </div>
        </template>
      </b-table>
    </scroll-wrapper>

    <TablePagination :count="machineTypesCount" />
    ...
    <MachineTypesModal
      ref="machine-types-modal"
      @updated="getMachineTypes"
    ></MachineTypesModal>
  </b-card>
</template>
...
import MachineTypesModal from '@/components/modals/MachineTypes'
  ...
  methods: {
    async getMachineTypes() {
      try {
        ...
        if (data.success) {
          this.machineTypes = data.results
          this.machineTypesCount = data.count
        }
    ...
    deleteMachineType(machineType) {
      this.$bvModal
        .msgBoxConfirm(`Are you sure you want to delete ${machineType.name}?`, {
    ...
    addMachineType() {
      this.$refs['machine-types-modal'].showModal(null)
    },
    editMachineType(terminal) {
      this.$refs['machine-types-modal'].showModal(terminal)
    },
    sortingChanged(sort) {
      this.$router.push({
        ...this.$route,
