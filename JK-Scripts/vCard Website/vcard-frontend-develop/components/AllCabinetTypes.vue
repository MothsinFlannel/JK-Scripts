<template>
  <b-card class="mb-4 shadow-sm">
    <template #header>
      ...
      >
        <p class="m-0">vCard Cabinet Types</p>
        <div
    ...
    >
      <div class="row mr-auto">
        <div class="col-12 d-flex flex-column">
          ...
          <b-pagination
            v-if="
              $route.query.limit > 0 && cabinetTypesCount > +$route.query.limit
            "
            :value="+$route.query.page"
            :total-rows="cabinetTypesCount"
        ...
        <template #cell(actions)="data">
          ...
          >
            <EditButton
              ...
              @click="editCabinetType(data.item)"
            />

            <DeleteButton @click="deleteCabinetType(data.item)" />
          </div>
        </template>
      </b-table>
    </scroll-wrapper>

    <TablePagination :count="cabinetTypesCount" />
...
import CabinetTypesModal from '@/components/modals/CabinetTypes'
  ...
  methods: {
    async getCabinetTypes() {
      try {
        ...
        if (data.success) {
          this.cabinetTypes = data.results
          this.cabinetTypesCount = data.count
        }
    ...
    deleteCabinetType(cabinetType) {
      this.$bvModal
        .msgBoxConfirm(`Are you sure you want to delete ${cabinetType.name}?`, {
    ...
    addCabinetType() {
      this.$refs['cabinet-types-modal'].showModal(null)
    },
    editCabinetType(terminal) {
      this.$refs['cabinet-types-modal'].showModal(terminal)
    },
    sortingChanged(sort) {
      this.$router.push({
        ...this.$route,
